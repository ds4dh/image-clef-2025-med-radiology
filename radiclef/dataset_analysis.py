...
from radiclef import ROCO_DATABASE_PATH, RESOURCES_DIR
...
cui_frequency_dict_test = {cui: 0 for cui in cui_alphabet}
for idx, cui_list in enumerate(dataset_test["cui_codes"]):
    for cui in cui_list:
        cui_frequency_dict_test[cui] += 1 / len(dataset_test)

print(cui_frequency_dict_train)
print(cui_frequency_dict_valid)
print(cui_frequency_dict_test)

plt.figure()
plot([v for v in cui_frequency_dict_train.values()])
plot([v for v in cui_frequency_dict_valid.values()])
plot([v for v in cui_frequency_dict_test.values()])
plt.show()
